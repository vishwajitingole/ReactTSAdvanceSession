import React from "react";
function Todo() {
    const[todo,setTodo]=React.useState([]);
    const [input,setInput]=React.useState("Enter your value");

    function handleClick(){
        console.log(input)
        setTodo([...todo,input])

        setInput("");
    }
    function gayabHoja(num){
       let result= todo.filter((e,i)=>i!==num);
       console.log(...result)
       setTodo([...result]);
    }

  return (
    <div className=" flex flex-col gap-5 items-center justify-center w-[60vw] h-[70vh] rounded-xl bg-stone-200">

        <div className="">  
            <input value={input} onChange={(e)=>setInput(e.target.value)} className=" bg-white rounded-lg w-50 px-2 py-2 border border-2 " type="text" /></div>
        <div className="">
            <button onClick={handleClick} className="w-full h-full px-1 py-2 bg-green-400 rounded-sm">Kuch bhi</button>
        </div>

        {todo.length>0?
        todo.map((e,i)=> <div key={i}>
            <div className="name">{e}</div>
                <button onClick={()=>gayabHoja(i)} className="w-full h-10 px-1 py-2 bg-red-400 rounded-sm">Delete</button>
            </div>):"Khaali h bhai"}
      


    </div>
  )
}

export default Todo